<x-app-layout>
    ...
    <style>
        ...
        .nw-card-heading { font-family: 'Playfair Display', serif; font-size: 1.6rem; font-weight: 400; color: var(--nw-text-primary, #1A1008); margin-bottom: 0.4rem; }
        .nw-card-id { font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 0.85rem; color: var(--sepia, #8B6F47); margin-bottom: 1.5rem; display: block; }
        ...
        .nw-input, .nw-textarea, .nw-select { width: 100%; background: rgba(255, 252, 245, 0.75); border: 1px solid rgba(139, 111, 71, 0.35); border-radius: 0; padding: 0.7rem 0.9rem; font-family: 'DM Sans', sans-serif; font-size: 0.875rem; color: var(--nw-text-primary, #1A1008); outline: none; transition: border-color 0.2s, box-shadow 0.2s; appearance: none; -webkit-appearance: none; }
        .nw-input:focus, .nw-textarea:focus, .nw-select:focus { border-color: var(--gold, #C9A84C); background: rgba(255, 252, 248, 0.95); box-shadow: 0 0 0 2px rgba(201, 168, 76, 0.12); }
            ...
            <div class="nw-form-card">
                <div class="overflow-hidden bg-white">
                    ...
                    <div class="nw-card-header">
                        ...
                        <h2 class="nw-card-heading">Edit Watch Unit</h2>
                        <span class="nw-card-id">ID #{{ $watchUnit->unitId }}</span>
                    </div>

                    <div class="nw-form-body">
                        <form method="POST" action="/watch-units/update/{{ $watchUnit->unitId }}">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="nw-select-wrap">
                                        <select id="modelId" name="modelId" class="nw-select" required>
                                            ...
                                            @foreach($watchModels as $model)
                                                <option value="{{ $model->modelId }}" {{ old('modelId', $watchUnit->modelId) == $model->modelId ? 'selected' : '' }}>
                                ...
                                <div class="nw-field">
                                    ...
                                    <div class="nw-select-wrap">
                                        <select id="condition" name="condition" class="nw-select">
                                            ...
                                            <option value="excellent" {{ old('condition', $watchUnit->condition) === 'excellent' ? 'selected' : '' }}>Excellent</option>
                                            <option value="good"      {{ old('condition', $watchUnit->condition) === 'good'      ? 'selected' : '' }}>Good</option>
                                            <option value="fair"      {{ old('condition', $watchUnit->condition) === 'fair'      ? 'selected' : '' }}>Fair</option>
                                            <option value="poor"      {{ old('condition', $watchUnit->condition) === 'poor'      ? 'selected' : '' }}>Poor</option>
                                            <option value="minus"     {{ old('condition', $watchUnit->condition) === 'minus'     ? 'selected' : '' }}>Minus</option>
                                        </select>
                                    </div>
                                ...
                                <div class="nw-field nw-field-full">
                                    ...
                                    <div class="nw-select-wrap">
                                        <select id="status" name="status" class="nw-select">
                                            <option value="">— None —</option>
                                            <option value="availabel"  {{ old('status', $watchUnit->status) === 'availabel'  ? 'selected' : '' }}>Available</option>
                                            <option value="reserved"   {{ old('status', $watchUnit->status) === 'reserved'   ? 'selected' : '' }}>Reserved</option>
                                            <option value="sold"       {{ old('status', $watchUnit->status) === 'sold'       ? 'selected' : '' }}>Sold</option>
                                            <option value="inServices" {{ old('status', $watchUnit->status) === 'inServices' ? 'selected' : '' }}>In Service</option>
                                        </select>
                                    </div>
                                ...
                                <div class="nw-field">
                                    ...
                                    <div class="nw-input-prefix-wrap">
                                        ...
                                        <input type="number" id="purchase_price" name="purchase_price" class="nw-input"
                                            value="{{ old('purchase_price', $watchUnit->purchase_price) }}" min="0">
                                    </div>
                                ...
                                <div class="nw-field">
                                    ...
                                    <div class="nw-input-prefix-wrap">
                                        ...
                                        <input type="number" id="asking_price" name="asking_price" class="nw-input"
                                            value="{{ old('asking_price', $watchUnit->asking_price) }}" min="0">
                                    </div>
                                ...
                                <div class="nw-field">
                                    ...
                                    <div class="nw-input-prefix-wrap">
                                        ...
                                        <input type="number" id="sold_price" name="sold_price" class="nw-input"
                                            value="{{ old('sold_price', $watchUnit->sold_price) }}" min="0">
                                    </div>
                            ...
                            <div class="nw-grid-2">
                                ...
                                <div class="nw-field">
                                    ...
                                    <input type="date" id="purchase_date" name="purchase_date" class="nw-input"
                                        value="{{ old('purchase_date', $watchUnit->purchase_date) }}">
                                ...
                                <div class="nw-field">
                                    ...
                                    <input type="date" id="sold_date" name="sold_date" class="nw-input"
                                        value="{{ old('sold_date', $watchUnit->sold_date) }}">
                                ...
                                <div class="nw-field nw-field-full">
                                    ...
                                    <input type="text" id="held" name="held" class="nw-input"
                                        value="{{ old('held', $watchUnit->held) }}">
                                    @error('held') <p class="nw-error">{{ $message }}</p> @enderror
                                </div>

                                <div class="nw-field nw-field-full">
                                    <label for="note" class="nw-label">Note</label>
                                    <textarea id="note" name="note" class="nw-textarea" maxlength="255">{{ old('note', $watchUnit->note) }}</textarea>
                            ...
                            <div class="nw-form-actions">
                                ...
                                <button type="submit" class="nw-btn-submit">Update Unit</button>
                            </div>
                        </form>
                    </div>
